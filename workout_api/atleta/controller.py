...
async def post (
      ...
      if not categoria:
            raise  HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                              detail=f' Esta categoria {categoria_name} nao foi encontrada')   
      ...
      if not centro_treinamento:
            raise  HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                              detail=f' Este centro de treinamento {centro_treinamento_name} nao esta cadastrado!')   
...
async def query (id: UUID4, db_session: DatabaseDependency) -> AtletaOut:
      ...
      if not atleta:
         raise  HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                              detail=f'Atleta nao econtrado pelo ID: {id}')   
